INCLUDE "constants.asm" ; no change for the variant 

; to add to data/pokemon/dex_entries: 

; duskull, bergmite, ralts, goomy, zorua, zoroark, gible


SECTION "CyndaquilPokedexEntry", ROMX
CyndaquilPokedexEntry::
INCLUDE "data/pokemon/dex_entries/155.asm"

SECTION "QuilavaPokedexEntry", ROMX
QuilavaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/156.asm"

SECTION "TyphlosionPokedexEntry", ROMX
TyphlosionPokedexEntry::
INCLUDE "data/pokemon/dex_entries/157.asm"

SECTION "RowletPokedexEntry", ROMX
RowletPokedexEntry::
INCLUDE "data/pokemon/dex_entries/302.asm"

SECTION "DartrixPokedexEntry", ROMX
DartrixPokedexEntry::
INCLUDE "data/pokemon/dex_entries/303.asm"

SECTION "DecidueyePokedexEntry", ROMX
DecidueyePokedexEntry::
INCLUDE "data/pokemon/dex_entries/304.asm"

SECTION "OshawottPokedexEntry", ROMX
OshawottPokedexEntry::
INCLUDE "data/pokemon/dex_entries/306.asm";;

SECTION "DewottPokedexEntry", ROMX
DewottPokedexEntry::
INCLUDE "data/pokemon/dex_entries/307.asm";;

SECTION "SamurottPokedexEntry", ROMX
SamurottPokedexEntry::
INCLUDE "data/pokemon/dex_entries/308.asm";;

SECTION "PidgeyPokedexEntry", ROMX
PidgeyPokedexEntry::
INCLUDE "data/pokemon/dex_entries/016.asm";;

SECTION "PidgeottoPokedexEntry", ROMX
PidgeottoPokedexEntry::
INCLUDE "data/pokemon/dex_entries/017.asm"

SECTION "PidgeotPokedexEntry", ROMX
PidgeotPokedexEntry::
INCLUDE "data/pokemon/dex_entries/018.asm"

SECTION "HoothootPokedexEntry", ROMX
HoothootPokedexEntry::
INCLUDE "data/pokemon/dex_entries/163.asm"

SECTION "NoctowlPokedexEntry", ROMX
NoctowlPokedexEntry::
INCLUDE "data/pokemon/dex_entries/019.asm"; new 

SECTION "PikachuPokedexEntry", ROMX
PikachuPokedexEntry::
INCLUDE "data/pokemon/dex_entries/025.asm";;

SECTION "RaichuPokedexEntry", ROMX
RaichuPokedexEntry::
INCLUDE "data/pokemon/dex_entries/026.asm";;

SECTION "NidoranPokedexEntry", ROMX
NidoranPokedexEntry::
INCLUDE "data/pokemon/dex_entries/nidoran.asm";;

SECTION "NidorinaPokedexEntry", ROMX
NidorinaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/030.asm";;

SECTION "NidoqueenPokedexEntry", ROMX
NidoqueenPokedexEntry::
INCLUDE "data/pokemon/dex_entries/031.asm";;

SECTION "NidorinoPokedexEntry", ROMX
NidorinoPokedexEntry::
INCLUDE "data/pokemon/dex_entries/033.asm";;

SECTION "NidokingPokedexEntry", ROMX
NidokingPokedexEntry::
INCLUDE "data/pokemon/dex_entries/034.asm";;

SECTION "ClefairyPokedexEntry", ROMX
ClefairyPokedexEntry::
INCLUDE "data/pokemon/dex_entries/035.asm";;

SECTION "ClefablePokedexEntry", ROMX
ClefablePokedexEntry::
INCLUDE "data/pokemon/dex_entries/036.asm";;

SECTION "VulpixPokedexEntry", ROMX
VulpixPokedexEntry::
INCLUDE "data/pokemon/dex_entries/037.asm"

SECTION "NinetalesPokedexEntry", ROMX
NinetalesPokedexEntry::
INCLUDE "data/pokemon/dex_entries/038.asm"

SECTION "ZubatPokedexEntry", ROMX
ZubatPokedexEntry::
INCLUDE "data/pokemon/dex_entries/041.asm"

SECTION "GolbatPokedexEntry", ROMX
GolbatPokedexEntry::
INCLUDE "data/pokemon/dex_entries/042.asm";;

SECTION "CrobatPokedexEntry", ROMX
CrobatPokedexEntry::
INCLUDE "data/pokemon/dex_entries/169.asm";;

SECTION "OddishPokedexEntry", ROMX
OddishPokedexEntry::
INCLUDE "data/pokemon/dex_entries/043.asm"

SECTION "GloomPokedexEntry", ROMX
GloomPokedexEntry::
INCLUDE "data/pokemon/dex_entries/044.asm"

SECTION "VileplumePokedexEntry", ROMX
VileplumePokedexEntry::
INCLUDE "data/pokemon/dex_entries/045.asm";;

SECTION "BellossomPokedexEntry", ROMX
BellossomPokedexEntry::
INCLUDE "data/pokemon/dex_entries/182.asm";;

SECTION "ParasPokedexEntry", ROMX
ParasPokedexEntry::
INCLUDE "data/pokemon/dex_entries/046.asm";;

SECTION "ParasectPokedexEntry", ROMX
ParasectPokedexEntry::
INCLUDE "data/pokemon/dex_entries/047.asm";;

SECTION "MankeyPokedexEntry", ROMX
MankeyPokedexEntry::
INCLUDE "data/pokemon/dex_entries/056.asm";;

SECTION "PrimeapePokedexEntry", ROMX
PrimeapePokedexEntry::
INCLUDE "data/pokemon/dex_entries/057.asm";;

SECTION "AnnihilapePokedexEntry", ROMX
AnnihilapePokedexEntry::
INCLUDE "data/pokemon/dex_entries/057_evolve.asm";;

SECTION "GrowlithePokedexEntry", ROMX
GrowlithePokedexEntry::
INCLUDE "data/pokemon/dex_entries/058.asm";;

SECTION "ArcaninePokedexEntry", ROMX
ArcaninePokedexEntry::
INCLUDE "data/pokemon/dex_entries/059.asm";;

SECTION "PoliwagPokedexEntry", ROMX
PoliwagPokedexEntry::
INCLUDE "data/pokemon/dex_entries/060.asm"

SECTION "PoliwhirlPokedexEntry", ROMX
PoliwhirlPokedexEntry::
INCLUDE "data/pokemon/dex_entries/061.asm"

SECTION "PoliwrathPokedexEntry", ROMX
PoliwrathPokedexEntry::
INCLUDE "data/pokemon/dex_entries/062.asm"

SECTION "PolitoedPokedexEntry", ROMX
PolitoedPokedexEntry::
INCLUDE "data/pokemon/dex_entries/186.asm";;

SECTION "AbraPokedexEntry", ROMX
AbraPokedexEntry::
INCLUDE "data/pokemon/dex_entries/063.asm";;

SECTION "KadabraPokedexEntry", ROMX
KadabraPokedexEntry::
INCLUDE "data/pokemon/dex_entries/064.asm";;

SECTION "AlakazamPokedexEntry", ROMX
AlakazamPokedexEntry::
INCLUDE "data/pokemon/dex_entries/065.asm";;

SECTION "MachopPokedexEntry", ROMX
MachopPokedexEntry::
INCLUDE "data/pokemon/dex_entries/066.asm";;

SECTION "MachokePokedexEntry", ROMX
MachokePokedexEntry::
INCLUDE "data/pokemon/dex_entries/067.asm";;

SECTION "MachampPokedexEntry", ROMX
MachampPokedexEntry::
INCLUDE "data/pokemon/dex_entries/068.asm";;

SECTION "BellsproutPokedexEntry", ROMX
BellsproutPokedexEntry::
INCLUDE "data/pokemon/dex_entries/069.asm";;

SECTION "WeepinbellPokedexEntry", ROMX
WeepinbellPokedexEntry::
INCLUDE "data/pokemon/dex_entries/070.asm";;

SECTION "VictreebelPokedexEntry", ROMX
VictreebelPokedexEntry::
INCLUDE "data/pokemon/dex_entries/071.asm";;

SECTION "TentacoolPokedexEntry", ROMX
TentacoolPokedexEntry::
INCLUDE "data/pokemon/dex_entries/072.asm";;

SECTION "TentacruelPokedexEntry", ROMX
TentacruelPokedexEntry::
INCLUDE "data/pokemon/dex_entries/073.asm";;

SECTION "GeodudePokedexEntry", ROMX
GeodudePokedexEntry::
INCLUDE "data/pokemon/dex_entries/074.asm";;

SECTION "GravelerPokedexEntry", ROMX
GravelerPokedexEntry::
INCLUDE "data/pokemon/dex_entries/075.asm";;

SECTION "GolemPokedexEntry", ROMX
GolemPokedexEntry::
INCLUDE "data/pokemon/dex_entries/076.asm";;

SECTION "PonytaPokedexEntry", ROMX
PonytaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/077.asm";;

SECTION "RapidashPokedexEntry", ROMX
RapidashPokedexEntry::
INCLUDE "data/pokemon/dex_entries/078.asm";;

SECTION "SlowpokePokedexEntry", ROMX
SlowpokePokedexEntry::
INCLUDE "data/pokemon/dex_entries/079.asm";;

SECTION "SlowbroPokedexEntry", ROMX
SlowbroPokedexEntry::
INCLUDE "data/pokemon/dex_entries/080.asm";;

SECTION "SlowkingPokedexEntry", ROMX
SlowkingPokedexEntry::
INCLUDE "data/pokemon/dex_entries/195.asm";;

SECTION "MagnemitePokedexEntry", ROMX
MagnemitePokedexEntry::
INCLUDE "data/pokemon/dex_entries/081.asm";;

SECTION "MagnetonPokedexEntry", ROMX
MagnetonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/082.asm"

SECTION "MagnezonePokedexEntry", ROMX
MagnezonePokedexEntry::
INCLUDE "data/pokemon/dex_entries/174.asm"

SECTION "GrimerPokedexEntry", ROMX
GrimerPokedexEntry::
INCLUDE "data/pokemon/dex_entries/088.asm"

SECTION "MukPokedexEntry", ROMX
MukPokedexEntry::
INCLUDE "data/pokemon/dex_entries/089.asm"

SECTION "GastlyPokedexEntry", ROMX
GastlyPokedexEntry::
INCLUDE "data/pokemon/dex_entries/092.asm"

SECTION "HaunterPokedexEntry", ROMX
HaunterPokedexEntry::
INCLUDE "data/pokemon/dex_entries/093.asm"

SECTION "GengarPokedexEntry", ROMX
GengarPokedexEntry::
INCLUDE "data/pokemon/dex_entries/094.asm"

SECTION "OnixPokedexEntry", ROMX
OnixPokedexEntry::
INCLUDE "data/pokemon/dex_entries/095.asm"

SECTION "SteelixPokedexEntry", ROMX
SteelixPokedexEntry::
INCLUDE "data/pokemon/dex_entries/206.asm";;

SECTION "VoltorbPokedexEntry", ROMX
VoltorbPokedexEntry::
INCLUDE "data/pokemon/dex_entries/100.asm"

SECTION "ElectrodePokedexEntry", ROMX
ElectrodePokedexEntry::
INCLUDE "data/pokemon/dex_entries/101.asm"

SECTION "ExeggcutePokedexEntry", ROMX
ExeggcutePokedexEntry::
INCLUDE "data/pokemon/dex_entries/102.asm"

SECTION "ExeggutorPokedexEntry", ROMX
ExeggutorPokedexEntry::
INCLUDE "data/pokemon/dex_entries/103.asm"

SECTION "HitmonleePokedexEntry", ROMX
HitmonleePokedexEntry::
INCLUDE "data/pokemon/dex_entries/106.asm"

SECTION "HitmonchanPokedexEntry", ROMX
HitmonchanPokedexEntry::
INCLUDE "data/pokemon/dex_entries/107.asm"

SECTION "HitmontopPokedexEntry", ROMX
HitmontopPokedexEntry::
INCLUDE "data/pokemon/dex_entries/237.asm";;

SECTION "KoffingPokedexEntry", ROMX
KoffingPokedexEntry::
INCLUDE "data/pokemon/dex_entries/109.asm";;

SECTION "WeezingPokedexEntry", ROMX
WeezingPokedexEntry::
INCLUDE "data/pokemon/dex_entries/110.asm";;

SECTION "RhyhornPokedexEntry", ROMX
RhyhornPokedexEntry::
INCLUDE "data/pokemon/dex_entries/111.asm"

SECTION "RhydonPokedexEntry", ROMX
RhydonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/112.asm"

SECTION "RhyperiorPokedexEntry", ROMX
RhyperiorPokedexEntry::
INCLUDE "data/pokemon/dex_entries/464.asm"

SECTION "ChanseyPokedexEntry", ROMX
ChanseyPokedexEntry::
INCLUDE "data/pokemon/dex_entries/113.asm";;

SECTION "BlisseyPokedexEntry", ROMX
BlisseyPokedexEntry::
INCLUDE "data/pokemon/dex_entries/242.asm";;

SECTION "TangelaPokedexEntry", ROMX
TangelaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/114.asm"

SECTION "TangrowthPokedexEntry", ROMX
TangrowthPokedexEntry::
INCLUDE "data/pokemon/dex_entries/465.asm"

SECTION "SeadraPokedexEntry", ROMX
SeadraPokedexEntry::
INCLUDE "data/pokemon/dex_entries/117.asm";;

SECTION "KingdraPokedexEntry", ROMX
KingdraPokedexEntry::
INCLUDE "data/pokemon/dex_entries/230.asm"

SECTION "ScytherPokedexEntry", ROMX
ScytherPokedexEntry::
INCLUDE "data/pokemon/dex_entries/123.asm";;

SECTION "ScizorPokedexEntry", ROMX
ScizorPokedexEntry::
INCLUDE "data/pokemon/dex_entries/212.asm"

SECTION "KleavorPokedexEntry", ROMX
KleavorPokedexEntry::
INCLUDE "data/pokemon/dex_entries/312.asm";;

SECTION "JynxPokedexEntry", ROMX
JynxPokedexEntry::
INCLUDE "data/pokemon/dex_entries/124.asm";;

SECTION "ElectabuzzPokedexEntry", ROMX
ElectabuzzPokedexEntry::
INCLUDE "data/pokemon/dex_entries/125.asm";;

SECTION "ElectivirePokedexEntry", ROMX
ElectivirePokedexEntry::
INCLUDE "data/pokemon/dex_entries/238.asm";;

SECTION "MagmarPokedexEntry", ROMX
MagmarPokedexEntry::
INCLUDE "data/pokemon/dex_entries/126.asm";;

SECTION "MagmortarPokedexEntry", ROMX
MagmortarPokedexEntry::
INCLUDE "data/pokemon/dex_entries/240.asm";;

SECTION "PinsirPokedexEntry", ROMX
PinsirPokedexEntry::
INCLUDE "data/pokemon/dex_entries/127.asm";;

SECTION "MinsirPokedexEntry", ROMX
MinsirPokedexEntry::
INCLUDE "data/pokemon/dex_entries/127_evolve.asm";;

SECTION "TaurosPokedexEntry", ROMX
TaurosPokedexEntry::
INCLUDE "data/pokemon/dex_entries/128.asm"

SECTION "MagikarpPokedexEntry", ROMX
MagikarpPokedexEntry::
INCLUDE "data/pokemon/dex_entries/129.asm"

SECTION "GyaradosPokedexEntry", ROMX
GyaradosPokedexEntry::
INCLUDE "data/pokemon/dex_entries/130.asm";;

SECTION "LaprasPokedexEntry", ROMX
LaprasPokedexEntry::
INCLUDE "data/pokemon/dex_entries/131.asm"

SECTION "EeveePokedexEntry", ROMX
EeveePokedexEntry::
INCLUDE "data/pokemon/dex_entries/133.asm"

SECTION "VaporeonPokedexEntry", ROMX
VaporeonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/134.asm"

SECTION "JolteonPokedexEntry", ROMX
JolteonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/135.asm"

SECTION "FlareonPokedexEntry", ROMX
FlareonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/136.asm"

SECTION "EspeonPokedexEntry", ROMX
EspeonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/196.asm"

SECTION "UmbreonPokedexEntry", ROMX
UmbreonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/197.asm";;

SECTION "LeafeonPokedexEntry", ROMX
LeafeonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/470.asm"

SECTION "GlaceonPokedexEntry", ROMX
GlaceonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/471.asm"

SECTION "SylveonPokedexEntry", ROMX
SylveonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/700.asm"

SECTION "PorygonPokedexEntry", ROMX
PorygonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/137.asm";;

SECTION "Porygon2PokedexEntry", ROMX
Porygon2PokedexEntry::
INCLUDE "data/pokemon/dex_entries/233.asm"

SECTION "Porygon_ZPokedexEntry", ROMX
Porygon_ZPokedexEntry::
INCLUDE "data/pokemon/dex_entries/474.asm"

SECTION "SnorlaxPokedexEntry", ROMX
SnorlaxPokedexEntry::
INCLUDE "data/pokemon/dex_entries/143.asm";;

SECTION "ChikoritaPokedexEntry", ROMX
ChikoritaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/152.asm"

SECTION "BayleefPokedexEntry", ROMX
BayleefPokedexEntry::
INCLUDE "data/pokemon/dex_entries/153.asm"

SECTION "MeganiumPokedexEntry", ROMX
MeganiumPokedexEntry::
INCLUDE "data/pokemon/dex_entries/154.asm";;

SECTION "TotodilePokedexEntry", ROMX
TotodilePokedexEntry::
INCLUDE "data/pokemon/dex_entries/158.asm"

SECTION "CroconawPokedexEntry", ROMX
CroconawPokedexEntry::
INCLUDE "data/pokemon/dex_entries/159.asm"

SECTION "FeraligatrPokedexEntry", ROMX
FeraligatrPokedexEntry::
INCLUDE "data/pokemon/dex_entries/160.asm";;

SECTION "SentretPokedexEntry", ROMX
SentretPokedexEntry::
INCLUDE "data/pokemon/dex_entries/161.asm"

SECTION "FurretPokedexEntry", ROMX
FurretPokedexEntry::
INCLUDE "data/pokemon/dex_entries/162.asm"

SECTION "LedybaPokedexEntry", ROMX
LedybaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/165.asm"

SECTION "LedianPokedexEntry", ROMX
LedianPokedexEntry::
INCLUDE "data/pokemon/dex_entries/166.asm"

SECTION "SpinarakPokedexEntry", ROMX
SpinarakPokedexEntry::
INCLUDE "data/pokemon/dex_entries/167.asm"

SECTION "AriadosPokedexEntry", ROMX
AriadosPokedexEntry::
INCLUDE "data/pokemon/dex_entries/168.asm"

SECTION "ChinchouPokedexEntry", ROMX
ChinchouPokedexEntry::
INCLUDE "data/pokemon/dex_entries/170.asm"

SECTION "LanturnPokedexEntry", ROMX
LanturnPokedexEntry::
INCLUDE "data/pokemon/dex_entries/171.asm";;

SECTION "TogeticPokedexEntry", ROMX
TogeticPokedexEntry::
INCLUDE "data/pokemon/dex_entries/119.asm";;

SECTION "TogekissPokedexEntry", ROMX
TogekissPokedexEntry::
INCLUDE "data/pokemon/dex_entries/468.asm"

SECTION "NatuPokedexEntry", ROMX
NatuPokedexEntry::
INCLUDE "data/pokemon/dex_entries/177.asm"

SECTION "XatuPokedexEntry", ROMX
XatuPokedexEntry::
INCLUDE "data/pokemon/dex_entries/178.asm";;

SECTION "MareepPokedexEntry", ROMX
MareepPokedexEntry::
INCLUDE "data/pokemon/dex_entries/179.asm"

SECTION "FlaaffyPokedexEntry", ROMX
FlaaffyPokedexEntry::
INCLUDE "data/pokemon/dex_entries/180.asm"

SECTION "AmpharosPokedexEntry", ROMX
AmpharosPokedexEntry::
INCLUDE "data/pokemon/dex_entries/181.asm";;

SECTION "MarillPokedexEntry", ROMX
MarillPokedexEntry::
INCLUDE "data/pokemon/dex_entries/021.asm"

SECTION "AzumarillPokedexEntry", ROMX
AzumarillPokedexEntry::
INCLUDE "data/pokemon/dex_entries/022.asm";;

SECTION "SudowoodoPokedexEntry", ROMX
SudowoodoPokedexEntry::
INCLUDE "data/pokemon/dex_entries/185.asm"

SECTION "SkiploomPokedexEntry", ROMX
SkiploomPokedexEntry::
INCLUDE "data/pokemon/dex_entries/313.asm"

SECTION "JumpluffPokedexEntry", ROMX
JumpluffPokedexEntry::
INCLUDE "data/pokemon/dex_entries/314.asm";;

SECTION "SunfloraPokedexEntry", ROMX
SunfloraPokedexEntry::
INCLUDE "data/pokemon/dex_entries/192.asm";;

SECTION "WooperPokedexEntry", ROMX
WooperPokedexEntry::
INCLUDE "data/pokemon/dex_entries/194.asm"

SECTION "QuagsirePokedexEntry", ROMX
QuagsirePokedexEntry::
INCLUDE "data/pokemon/dex_entries/190.asm"

SECTION "MurkrowPokedexEntry", ROMX
MurkrowPokedexEntry::
INCLUDE "data/pokemon/dex_entries/198.asm"

SECTION "HonchkrowPokedexEntry", ROMX
HonchkrowPokedexEntry::
INCLUDE "data/pokemon/dex_entries/430.asm"

SECTION "MisdreavusPokedexEntry", ROMX
MisdreavusPokedexEntry::
INCLUDE "data/pokemon/dex_entries/200.asm"

SECTION "MismagiusPokedexEntry", ROMX
MismagiusPokedexEntry::
INCLUDE "data/pokemon/dex_entries/429.asm"

SECTION "YanmaPokedexEntry", ROMX
YanmaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/193.asm"

SECTION "YanmegaPokedexEntry", ROMX
YanmegaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/469.asm";;

SECTION "UnownPokedexEntry", ROMX
UnownPokedexEntry::
INCLUDE "data/pokemon/dex_entries/201.asm"

SECTION "GirafarigPokedexEntry", ROMX
GirafarigPokedexEntry::
INCLUDE "data/pokemon/dex_entries/203.asm"

SECTION "FarigirafPokedexEntry", ROMX
FarigirafPokedexEntry::
INCLUDE "data/pokemon/dex_entries/farigiraf.asm"

SECTION "PinecoPokedexEntry", ROMX
PinecoPokedexEntry::
INCLUDE "data/pokemon/dex_entries/204.asm"

SECTION "ForretressPokedexEntry", ROMX
ForretressPokedexEntry::
INCLUDE "data/pokemon/dex_entries/205.asm"

SECTION "DunsparcePokedexEntry", ROMX
DunsparcePokedexEntry::
INCLUDE "data/pokemon/dex_entries/206.asm"

SECTION "DudunsparcePokedexEntry", ROMX
DudunsparcePokedexEntry::
INCLUDE "data/pokemon/dex_entries/dudunsparce.asm" ; done  

SECTION "GligarPokedexEntry", ROMX
GligarPokedexEntry::
INCLUDE "data/pokemon/dex_entries/207.asm"

SECTION "GliscorPokedexEntry", ROMX
GliscorPokedexEntry::
INCLUDE "data/pokemon/dex_entries/472.asm"

SECTION "SnubbullPokedexEntry", ROMX
SnubbullPokedexEntry::
INCLUDE "data/pokemon/dex_entries/209.asm"

SECTION "GranbullPokedexEntry", ROMX
GranbullPokedexEntry::
INCLUDE "data/pokemon/dex_entries/210.asm"

SECTION "QwilfishPokedexEntry", ROMX
QwilfishPokedexEntry::
INCLUDE "data/pokemon/dex_entries/211.asm"

SECTION "OverqwilPokedexEntry", ROMX
OverqwilPokedexEntry::
INCLUDE "data/pokemon/dex_entries/315.asm";;

SECTION "ShucklePokedexEntry", ROMX
ShucklePokedexEntry::
INCLUDE "data/pokemon/dex_entries/213.asm"

SECTION "HeracrossPokedexEntry", ROMX
HeracrossPokedexEntry::
INCLUDE "data/pokemon/dex_entries/214.asm"

SECTION "SneaselPokedexEntry", ROMX
SneaselPokedexEntry::
INCLUDE "data/pokemon/dex_entries/215.asm"

SECTION "WeavilePokedexEntry", ROMX
WeavilePokedexEntry::
INCLUDE "data/pokemon/dex_entries/461.asm"

SECTION "SneaslerPokedexEntry", ROMX
SneaslerPokedexEntry::
INCLUDE "data/pokemon/dex_entries/316.asm";;

SECTION "TeddiursaPokedexEntry", ROMX
TeddiursaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/216.asm"

SECTION "UrsaringPokedexEntry", ROMX
UrsaringPokedexEntry::
INCLUDE "data/pokemon/dex_entries/217.asm"

SECTION "UrsalunaPokedexEntry", ROMX
UrsalunaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/317.asm";;

SECTION "SlugmaPokedexEntry", ROMX
SlugmaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/218.asm";;

SECTION "MagcargoPokedexEntry", ROMX
MagcargoPokedexEntry::
INCLUDE "data/pokemon/dex_entries/219.asm";;

SECTION "SwinubPokedexEntry", ROMX
SwinubPokedexEntry::
INCLUDE "data/pokemon/dex_entries/220.asm"

SECTION "PiloswinePokedexEntry", ROMX
PiloswinePokedexEntry::
INCLUDE "data/pokemon/dex_entries/221.asm"

SECTION "MamoswinePokedexEntry", ROMX
MamoswinePokedexEntry::
INCLUDE "data/pokemon/dex_entries/473.asm"

SECTION "CorsolaPokedexEntry", ROMX
CorsolaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/222.asm";;;

SECTION "RemoraidPokedexEntry", ROMX
RemoraidPokedexEntry::
INCLUDE "data/pokemon/dex_entries/223.asm"

SECTION "OctilleryPokedexEntry", ROMX
OctilleryPokedexEntry::
INCLUDE "data/pokemon/dex_entries/224.asm"

SECTION "MantinePokedexEntry", ROMX
MantinePokedexEntry::
INCLUDE "data/pokemon/dex_entries/226.asm"

SECTION "SkarmoryPokedexEntry", ROMX
SkarmoryPokedexEntry::
INCLUDE "data/pokemon/dex_entries/227.asm"

SECTION "HoundourPokedexEntry", ROMX
HoundourPokedexEntry::
INCLUDE "data/pokemon/dex_entries/228.asm"

SECTION "HoundoomPokedexEntry", ROMX
HoundoomPokedexEntry::
INCLUDE "data/pokemon/dex_entries/229.asm"

SECTION "PhanpyPokedexEntry", ROMX
PhanpyPokedexEntry::
INCLUDE "data/pokemon/dex_entries/231.asm"

SECTION "DonphanPokedexEntry", ROMX
DonphanPokedexEntry::
INCLUDE "data/pokemon/dex_entries/232.asm"

SECTION "StantlerPokedexEntry", ROMX
StantlerPokedexEntry::
INCLUDE "data/pokemon/dex_entries/318.asm";;

SECTION "WyrdeerPokedexEntry", ROMX
WyrdeerPokedexEntry::
INCLUDE "data/pokemon/dex_entries/319.asm";;

SECTION "MiltankPokedexEntry", ROMX
MiltankPokedexEntry::
INCLUDE "data/pokemon/dex_entries/241.asm";;

SECTION "GrotlePokedexEntry", ROMX
GrotlePokedexEntry::
INCLUDE "data/pokemon/dex_entries/320.asm"

SECTION "TorterraPokedexEntry", ROMX
TorterraPokedexEntry::
INCLUDE "data/pokemon/dex_entries/321.asm";;

SECTION "MonfernoPokedexEntry", ROMX;;
MonfernoPokedexEntry::
INCLUDE "data/pokemon/dex_entries/322.asm"

SECTION "InfernapePokedexEntry", ROMX
InfernapePokedexEntry::
INCLUDE "data/pokemon/dex_entries/323.asm";;

SECTION "PrinplupPokedexEntry", ROMX
PrinplupPokedexEntry::
INCLUDE "data/pokemon/dex_entries/324.asm"

SECTION "EmpoleonPokedexEntry", ROMX
EmpoleonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/325.asm";;

SECTION "StaraviaPokedexEntry", ROMX
StaraviaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/326.asm"

SECTION "StaraptorPokedexEntry", ROMX
StaraptorPokedexEntry::
INCLUDE "data/pokemon/dex_entries/327.asm";;

SECTION "ShinxPokedexEntry", ROMX
ShinxPokedexEntry::
INCLUDE "data/pokemon/dex_entries/shinx.asm"

SECTION "LuxioPokedexEntry", ROMX
LuxioPokedexEntry::
INCLUDE "data/pokemon/dex_entries/328.asm"

SECTION "LuxrayPokedexEntry", ROMX
LuxrayPokedexEntry::
INCLUDE "data/pokemon/dex_entries/329.asm";;

SECTION "CranidosPokedexEntry", ROMX
CranidosPokedexEntry::
INCLUDE "data/pokemon/dex_entries/cranidos.asm"

SECTION "RampardosPokedexEntry", ROMX
RampardosPokedexEntry::
INCLUDE "data/pokemon/dex_entries/332.asm"

SECTION "ShieldonPokedexEntry", ROMX
ShieldonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/shieldon.asm";;

SECTION "BastiodonPokedexEntry", ROMX
BastiodonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/333.asm";;

SECTION "VespiquenPokedexEntry", ROMX
VespiquenPokedexEntry::
INCLUDE "data/pokemon/dex_entries/334.asm";;

SECTION "AipomPokedexEntry", ROMX
AipomPokedexEntry::
INCLUDE "data/pokemon/dex_entries/aipom.asm";;

SECTION "AmbipomPokedexEntry", ROMX
AmbipomPokedexEntry::
INCLUDE "data/pokemon/dex_entries/335.asm";;

SECTION "DrifloonPokedexEntry", ROMX
DrifloonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/drifloon.asm";;

SECTION "DrifblimPokedexEntry", ROMX
DrifblimPokedexEntry::
INCLUDE "data/pokemon/dex_entries/336.asm";;

SECTION "BronzorPokedexEntry", ROMX
BronzorPokedexEntry::
INCLUDE "data/pokemon/dex_entries/339.asm"

SECTION "BronzongPokedexEntry", ROMX
BronzongPokedexEntry::
INCLUDE "data/pokemon/dex_entries/340.asm";;

SECTION "SpiritombPokedexEntry", ROMX
SpiritombPokedexEntry::
INCLUDE "data/pokemon/dex_entries/341.asm"

SECTION "GabitePokedexEntry", ROMX
GabitePokedexEntry::
INCLUDE "data/pokemon/dex_entries/gible.asm"

SECTION "GabitePokedexEntry", ROMX
GabitePokedexEntry::
INCLUDE "data/pokemon/dex_entries/342.asm"

SECTION "GarchompPokedexEntry", ROMX
GarchompPokedexEntry::
INCLUDE "data/pokemon/dex_entries/343.asm";;

SECTION "LucarioPokedexEntry", ROMX
LucarioPokedexEntry::
INCLUDE "data/pokemon/dex_entries/344.asm"

SECTION "HippopotasPokedexEntry", ROMX
HippopotasPokedexEntry::
INCLUDE "data/pokemon/dex_entries/hippopotas.asm"

SECTION "HippowdonPokedexEntry", ROMX
HippowdonPokedexEntry::
INCLUDE "data/pokemon/dex_entries/345.asm"

SECTION "CroagunkPokedexEntry", ROMX
CroagunkPokedexEntry::
INCLUDE "data/pokemon/dex_entries/croagunk.asm"

SECTION "ToxicroakPokedexEntry", ROMX
ToxicroakPokedexEntry::
INCLUDE "data/pokemon/dex_entries/347.asm"

SECTION "SnoverPokedexEntry", ROMX
SnoverPokedexEntry::
INCLUDE "data/pokemon/dex_entries/348.asm"

SECTION "AbomasnowPokedexEntry", ROMX
AbomasnowPokedexEntry::
INCLUDE "data/pokemon/dex_entries/349.asm";;

SECTION "DuskullPokedexEntry", ROMX
DuskullPokedexEntry::
INCLUDE "data/pokemon/dex_entries/duskull.asm"

SECTION "DusclopsPokedexEntry", ROMX
DusclopsPokedexEntry::
INCLUDE "data/pokemon/dex_entries/350.asm"

SECTION "DusknoirPokedexEntry", ROMX
DusknoirPokedexEntry::
INCLUDE "data/pokemon/dex_entries/351.asm";;

SECTION "FroslassPokedexEntry", ROMX
FroslassPokedexEntry::
INCLUDE "data/pokemon/dex_entries/353.asm"

SECTION "RotomPokedexEntry", ROMX
RotomPokedexEntry::
INCLUDE "data/pokemon/dex_entries/352.asm";;

SECTION "BergmitePokedexEntry", ROMX
BergmitePokedexEntry::
INCLUDE "data/pokemon/dex_entries/bergmite.asm";;

SECTION "AvaluggPokedexEntry", ROMX
AvaluggPokedexEntry::
INCLUDE "data/pokemon/dex_entries/354.asm";;

SECTION "BasculegionPokedexEntry", ROMX
BasculegionPokedexEntry::
INCLUDE "data/pokemon/dex_entries/355.asm";;

SECTION "BraviaryPokedexEntry", ROMX
BraviaryPokedexEntry::
INCLUDE "data/pokemon/dex_entries/356.asm";;

SECTION "RaltsPokedexEntry", ROMX
RaltsPokedexEntry::
INCLUDE "data/pokemon/dex_entries/ralts.asm"

SECTION "KirliaPokedexEntry", ROMX
KirliaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/357.asm"

SECTION "GardevoirPokedexEntry", ROMX
GardevoirPokedexEntry::
INCLUDE "data/pokemon/dex_entries/358.asm";;

SECTION "GalladePokedexEntry", ROMX
GalladePokedexEntry::
INCLUDE "data/pokemon/dex_entries/359.asm";;

SECTION "GoomyPokedexEntry", ROMX
SliggooPokedexEntry::
INCLUDE "data/pokemon/dex_entries/goomy.asm";;

SECTION "SliggooPokedexEntry", ROMX
SliggooPokedexEntry::
INCLUDE "data/pokemon/dex_entries/360.asm";;

SECTION "GoodraPokedexEntry", ROMX
GoodraPokedexEntry::
INCLUDE "data/pokemon/dex_entries/361.asm"

SECTION "ZoruaPokedexEntry", ROMX
ZoruaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/zorua.asm"

SECTION "ZoroarkPokedexEntry", ROMX
ZoroarkPokedexEntry::
INCLUDE "data/pokemon/dex_entries/362.asm"

SECTION "DratiniPokedexEntry", ROMX
DratiniPokedexEntry::
INCLUDE "data/pokemon/dex_entries/147.asm";;

SECTION "DragonairPokedexEntry", ROMX
DragonairPokedexEntry::
INCLUDE "data/pokemon/dex_entries/148.asm";;

SECTION "DragonitePokedexEntry", ROMX
DragonitePokedexEntry::
INCLUDE "data/pokemon/dex_entries/149.asm";;

SECTION "LarvitarPokedexEntry", ROMX
LarvitarPokedexEntry::
INCLUDE "data/pokemon/dex_entries/246.asm";;

SECTION "PupitarPokedexEntry", ROMX
PupitarPokedexEntry::
INCLUDE "data/pokemon/dex_entries/247.asm";;

SECTION "TyranitarPokedexEntry", ROMX
TyranitarPokedexEntry::
INCLUDE "data/pokemon/dex_entries/248.asm";;

SECTION "RaikouPokedexEntry", ROMX
RaikouPokedexEntry::
INCLUDE "data/pokemon/dex_entries/243.asm";;

SECTION "EnteiPokedexEntry", ROMX
EnteiPokedexEntry::
INCLUDE "data/pokemon/dex_entries/244.asm";;

SECTION "SuicunePokedexEntry", ROMX
SuicunePokedexEntry::
INCLUDE "data/pokemon/dex_entries/245.asm";;

SECTION "HeatranPokedexEntry", ROMX
HeatranPokedexEntry::
INCLUDE "data/pokemon/dex_entries/363.asm";;

SECTION "LugiaPokedexEntry", ROMX
LugiaPokedexEntry::
INCLUDE "data/pokemon/dex_entries/249.asm";;

SECTION "Ho_OhPokedexEntry", ROMX
Ho_OhPokedexEntry::
INCLUDE "data/pokemon/dex_entries/250.asm";;

SECTION "CelebiPokedexEntry", ROMX
CelebiPokedexEntry::
INCLUDE "data/pokemon/dex_entries/251.asm";;
